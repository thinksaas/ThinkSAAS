...
//最新书籍 
$arrApples = $db->findAll("select * from ".dbprefix."apple order by addtime desc limit 6");

foreach($arrApples as $key=>$item){
	$arrApple[] = $item;
	$index = $db->findAll("select * from ".dbprefix."apple_index where `appleid`='".$item['appleid']."'");
...
//最新点评
$arrReviews = $db->findAll("select * from ".dbprefix."apple_review order by addtime desc limit 6");
