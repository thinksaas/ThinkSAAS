...
switch($ts){
	...
	case "do":
		...
		if($email=='' || $pwd=='') tsNotice(L::login_entryempty);
		
		$isEmail = $db->once_fetch_assoc("select count(*) from ".dbprefix."user where `email`='$email'");
		
		if($isEmail['count(*)'] == 0) tsNotice(L::login_emailnotregistered);
		
		$strUser = $db->once_fetch_assoc("select * from ".dbprefix."user where `email`='$email'");
			
		if(md5($strUser['salt'].$pwd)!==$strUser['pwd']) tsNotice(L::login_pwderror);	
