...
if(intval($TS_USER['user']['userid']) == $strGroup['userid'] || intval($TS_USER['user']['isadmin']==1)){
	...
	switch($ts){
		...
		case "cate":
			...
			$groupcateindexnum = $db->once_num_rows("select * from ".dbprefix."discuss_cates_index where groupid='$groupid'");
			
			if($groupcateindexnum > 0){
				$arrGroupCateIndex = $db->fetch_all_assoc("select * from ".dbprefix."discuss_cates_index where groupid='$groupid'");
				
				
				foreach($arrGroupCateIndex as $key=>$item){
					$strCate = $db->once_fetch_assoc("select * from ".dbprefix."discuss_cates where cateid='".$item['cateid']."'");
					$arrCate[] = $strCate;
				}
			}
		...
		case "type":
			//调出类型
			$arrGroupType = $db->fetch_all_assoc("select * from ".dbprefix."discuss_topics_type where groupid='".$strGroup['groupid']."'");
		...
		case "type_add":
			...
			if($typename != '')
			  $db->query("insert into ".dbprefix."discuss_topics_type (`groupid`,`typename`) values ('$groupid','$typename')");
			
			header("Location: ".SITE_URL.tsurl('discuss','edit',array('ts'=>'type','groupid'=>$groupid)));
			break;
