...
class tag{
		...
		if($objname != '' && $idname != '' && $objid!='' && $tags!=''){
			...
			foreach($arrTag as $item){
				...
				if(strlen($tagname) < '32' && $tagname != ''){
					$uptime = time();
					$tagcount = $this->db->findCount("select * from ".dbprefix."tag where tagname='".$tagname."'");
					
					if($tagcount == '0'){
						$this->db->query("INSERT INTO ".dbprefix."tag (`tagname`,`uptime`) VALUES ('".$tagname."','".$uptime."')");
						$tagid = $this->db->insert_id();
						
						$tagIndexCount = $this->db->findCount("select * from ".dbprefix."tag_".$objname."_index where ".$idname."='".$objid."' and tagid='".$tagid."'");
						if($tagIndexCount == '0'){
						...
						$tagIndexCount = $this->db->findCount("select * from ".dbprefix."tag_".$objname."_index where ".$idname."='".$objid."' and tagid='".$tagData['tagid']."'");
						if($tagIndexCount == '0'){
	...
	function getObjTagByObjid($objname,$idname,$objid){
		$arrTagIndex = $this->db->findAll("select * from ".dbprefix."tag_".$objname."_index where ".$idname."='$objid'");
	...
	function getOneTag($tagid){
		$tagData = $this->db->find("select * from ".dbprefix."tag where tagid='$tagid'");
	...
	function getTagId($tagname){
		$strTag = $this->db->find("select tagid from ".dbprefix."tag where `tagname`='$tagname'");
